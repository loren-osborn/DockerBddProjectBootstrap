...
PROJECT_PREFERED_COMMAND_RESOLUTION := FN__PREFER_BINARY_WHICH
...
$(call \
	...
	docker-compose,$\
	DOCKER_COMPOSE,$\
	Docker Compose,Docker must be installed. docker-compose binary not found in current PATH.$\
)
...
.PHONY: all default test test_cachable test_force clean dist_clean dev_containers verbose verbose_debug
